...
def cifar10_resnet_v2_generator(resnet_size, num_classes, data_format=None):
  ...
  Args:
    resnet_size: A string int-int for the size of the WideResNet model
     and string int for ResNet Model.
  ...
  if not isinstance(resnet_size, str):
    raise ValueError('resnet_size is not a string')

  if "-" in resnet_size:
    resnet_size, k = [int(x) for x in resnet_size.split("-")]
    if resnet_size % 6 != 4 or k <= 0:
      raise ValueError('resnet_size must be 6n + 4 and k>0 for wideresnet:', resnet_size)
    num_blocks = (resnet_size - 4) // 6
  else:
